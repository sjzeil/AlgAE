...
/**
 * The "normal" Client for AlgAE animations, this portrays a series of state snapshots
 * via a GUI with controls for selecting algorithms, starting, pausing, and rewinding playback.
 *   
 * @author zeil
 *
 */
public class GUIClient extends AppletMenuSupport implements Client {
	...
	public GUIClient(ServerProxy server)
	{
		super();
	...
	private class ClientThread extends Thread {
		...
		public void run() {
			synchronized (client) {
				...
				try {
					getServerProxy().sendToServer(startMsg);
				} catch (InterruptedException e) {
					ioPane.print("Connection to server has failed\n");
				}
			}

	...
	public void buildMenu() {
		...
		registerStartingAction(new MenuFunction() {
			...
			public void selected() {
				getMemoryModel().render(list.getClass(), new LinkedListRenderer<String>(true, false, self));
				globalVar("list", list);
		...
		register ("find", new MenuFunction() {
			@Override
			public void selected() {
				ActivationRecord arec = LocalJavaAnimation.activate(getClass());
				arec.breakHere("Prompt for input");
				String value = promptForInput("Search for ...", "[0-9]+");
				ListIterator<String> it0 = list.listIterator();
				ListIterator<String> it= new LinkedIterator<String>(it0, list, self);
		...
		register ("lots of locals", new MenuFunction() {
			@Override
			public void selected() {
				ActivationRecord arec = LocalJavaAnimation.activate(list);

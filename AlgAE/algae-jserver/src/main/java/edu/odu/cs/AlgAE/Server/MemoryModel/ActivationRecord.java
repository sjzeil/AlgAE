...
public class ActivationRecord implements ContextAware, CanBeRendered<ActivationRecord> {
	...
	public String promptForInput(String prompt, String requiredPattern) {
		LocalJavaAnimation context = LocalJavaAnimation.algae();
		String response = JOptionPane.showInputDialog(context, prompt, "Input Requested", JOptionPane.QUESTION_MESSAGE);
		while (!response.matches(requiredPattern)) {
			response = JOptionPane.showInputDialog(context, prompt, "Try again", JOptionPane.WARNING_MESSAGE);
		}
		return response;
	}
